...
@Component({
    selector: 'app-byleader',
    templateUrl: './byleader.component.html',
    styleUrls: ['./byleader.component.scss'],
})
export class ByleaderComponent {
        ...
        this.acti_service
            ...
            .subscribe((result: any) => {
                if (result.result == EnumStatus.ok) {
                    this.ListLeader = [];
                    this.ListLeader = result.data.data;
                }
            });
    }
    openDetail(item: any) {
        const token = Helper.getPrams('token', this.activate);
        const url =
            '/activation/sellout-detail-by-employee/' +
            item.employee_id +
            '/?token=' +
            token;

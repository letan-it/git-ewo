...
export class WorksDetailComponent implements OnInit {
    ...
    loadItemKPI() {
        this._service
            .Report_CheckKpi(this.inValue.report_id)
            .subscribe((data: any) => {
                const result = data.data;
                let kpiChecks = [
                    { name: 'POSM', method: () => this.loadDataPOSM() },
                    { name: 'OSA', method: () => this.loadDataOSA() },
                    {
                        name: 'DISPLAY',
                        method: () =>
                    ...
                    },
                    {
                        name: 'ACTIVATION',
                        method: () =>
                            this.loadDataActivation(this.inValue.report_id),
                    },
                    {
                        name: 'SELLIN',
                        method: () =>
                    ...
                    { name: 'OOL', method: () => this.loadDataKpiOOL() },
                    {
                        name: 'FIELCOACHING',
                        method: () =>
                ...
                kpiChecks.forEach((kpi: any) => {
                    const check = result.find((e: any) => e.kpi === kpi.name);
                    kpi.NameVN = check.NameVN ? check.NameVN : kpi.name;
                    kpi.NameEN = check.NameEN ? check.NameEN : kpi.name;
                    if (check && check.check === 1) {
                        try {
                            kpi.method();
                        } catch (error) {
                            console.log('Error load kpi ' + kpi.name);
                        }
                        let tmp = {
                            key: kpi.name,
                            en: check.NameEN ? check.NameEN : kpi.name,
                            vn: check.NameVN ? check.NameVN : kpi.name,
                        };
                        this.language_report.push(tmp);
                    }
                });
            });
        //check shop
        ...
        this.osaService
            ...
            .subscribe((data: any) => {
                if (data.result == EnumStatus.ok) {
                    ...
                    this.inValue.data_osa.osA_Detail.forEach((e: any) => {
                        e.tooltip =
                            e.updated_code != null
                                ? 'Updated by: ' +
                                  e.updated_name +
                    ...
                    this.inValue.data_osa.osA_Results.forEach(
                        (element: any) => {
                            ...
                            element.detail.fromOOC =
                                element.detail.filter((x: any) => x.ooc != null)
                                    .length > 0 ||
                                element.detail[0].check_ooc == 1
                                    ? 1
                                    : 0;
                            element.detail.fromOOS =
                                element.detail.filter((x: any) => x.oos != null)
                                    .length > 0 ||
                            ...
                            element.detail.fromQuantity =
                                ...
                                ).length > 0 || element.detail[0].check_qty == 1
                                    ? 1
                                    : 0;
        ...
        this.pomService
            ...
            .subscribe((data: any) => {
                ...
                if (data.result == EnumStatus.ok) {
                    ...
                    this.inValue.data_posm.posM_Results.forEach((e: any) => {
                        e.tooltip =
                            e.updated_code != null
                                ? 'Updated by: ' +
                                  e.updated_name +
    ...
    loadDataInventory(project_id: any, report_id: number) {
        this.inventoryService
            .Results_GetList(project_id, report_id)
            .subscribe((data: any) => {
                if (data.result == EnumStatus.ok) {
                    this.inValue.data_inventory = data.data.result;

                    let listProduct = [] as any;
                    this.inValue.data_inventory.forEach((result: any) => {
                        result.details = data.data.details.filter(
                            (d: any) => d.result_id == result.result_id
                        );
                        result.details.forEach((d1: any) => {
                            d1.his =
                                Helper.IsNull(d1.his) != true
                                    ? JSON.parse(d1.his)
                                    : [];
                            listProduct.push({
                                name: `[${d1.product_id}] - ${d1.product_name}`,
                                code: d1.product_code,
                            });
                        });
                        result.listProduct = listProduct;
                        result.image = data.data.image.filter(
                            (i: any) => i.result_id == result.result_id
                        );
                    });
                    this.inValue.data_inventory.forEach((x: any) => {
                        x.details.forEach((e: any) => {
                            e.tooltip =
                                e.updated_code != null
                                    ? 'Updated by: ' +
                                      e.updated_name +
                    ...
                    try {
                        const editData = this.dataQC.filter(
                            (qc: any) => qc.KPI == 'INVENTORY'
                        )[0].is_edit_data;
                        this.inValue.data_inventory.is_edit_data = editData;

                        // this.inValue.data_inventory.result.forEach(
                        //     (element: any) => {
                        //         try {
                        //             element.is_edit_data = editData;
                        //         } catch (error) {
    ...
    loadDataCoaching(project_id: any, report_id: number) {
        this.fieldCoachingService
            ...
            .subscribe((data: any) => {
                if (data.result == EnumStatus.ok) {
                    this.inValue.data_filedCoaching = data.data;
                }
            });
    }

    loadDataSOS(project_id: any, report_id: number) {
        this.sosService
            .Results_GetList(project_id, report_id)
            .subscribe((data: any) => {
                if (data.result == EnumStatus.ok) {
                    this.inValue.data_sos = data.data.result;
                    // this.inValue.data_sos.details.forEach((e: any) => {
                    //     e.tooltip =
                    //         e.update_by != null
                    //             ? 'Updated by: ' +
                    //               e.updated_name +
                    ...
                    let listSOS = [] as any;
                    this.inValue.data_sos.forEach((result: any) => {
                        result.details = data.data.details.filter(
                            (d: any) => d.result_id == result.result_id
                        );
                        result.details.forEach((d1: any) => {
                            d1.his =
                                Helper.IsNull(d1.his) != true
                                    ...
                                    : [];
                            listSOS.push({
                                name: `[${d1.sos_id}] - ${d1.sos_type} - ${d1.sos_code} - ${d1.sos_name}`,
                                code: d1.sos_code,
                            });
                        });
                        result.listSOS = listSOS;
                        result.image = data.data.image.filter(
                            (i: any) => i.result_id == result.result_id
                        );
                    });
                    this.inValue.data_sos.forEach((x: any) => {
                        x.details.forEach((e: any) => {
                            e.tooltip =
                                e.updated_code != null
                                    ? 'Updated by: ' +
                    ...
                    try {
                        ...
                        this.inValue.data_sos.is_edit_data = editData;

                        // this.inValue.data_sos.result.forEach(
                        //     (element: any) => {
                        //         try {
                        //             element.is_edit_data = editData;
                        //         } catch (error) {
    ...
    selectedFilterWorkShift(event: any) {
        event.value.atd_type_time =
            event.value.atd_type + ' - ' + event.value.atd_time;
                ...
                .subscribe((data: any) => {
                    if (data.result == EnumStatus.ok) {
                        ...
                        if (data.data.length > 0) {
                            ...
                            this.ListATT_getItem.report_id =
                                this.inValue.report_id;
                            this.listphoto = [];
                            for (
                                var i = 0;
                            ...
                            try {
                                ...
                                if (
                                    Helper.IsNull(this.ListATT_getItemOne) !=
                                    true
                                ) {
                                    this.ListATT_getItemOne.is_edit_data =
                                        editData;
                                }
                            } catch (error) {}
                            try {
                                ...
                                if (
                                    Helper.IsNull(this.ListATT_getItemTwo) !=
                                    true
                                ) {
                                    this.ListATT_getItemTwo.is_edit_data =
                                        editData;
                                }
                            } catch (error) {}
                            try {
                                ...
                                if (
                                    Helper.IsNull(this.ListATT_getItemThree) !=
                                    true
                                ) {
                                    this.ListATT_getItemThree.is_edit_data =
                                        editData;
    ...
    loadDataActivationItem(
        ...
        this.activationService
            .work_item_GetList(Helper.ProjectID(), work_id, work_item_id, key)
            .subscribe((data: any) => {
                if (data.result == EnumStatus.ok) {
                    if (key == 'SURVEY') {
                        if (Helper.IsNull(data.data.data) != true) {
                            step.item = data.data.data.filter(
                                (x: any) =>
                                    x.typeOf != 'alert' && x.typeOf != 'final'
                            );
                            step.item = step.item.map((x: any) => ({
                                ...x,
                                _value_string: x.value_string,
                                toolTip: `Create By: [${x.created_by}] - ${x.employee_code} - ${x.employee_name} | Create Date: ${x.created_date}`,
                            }));
                        }

                        step.item.forEach((i: any) => {
                            if (
                                i.question_type == 'multi-select' &&
                                i.check_display == 1
                            ) {
                                const listMulti = step.item.filter(
                                    (ws: any) =>
                                        ws.id != i.id &&
                                        ws.work_id == i.work_id &&
                                        ws.work_item_id == i.work_item_id &&
                                        ws.survey_id == i.survey_id &&
                                        ws.question_id == i.question_id
                                );
                                listMulti.forEach((multi: any) => {
                                    i._value_string += `, ${multi.value_string}`;
                                });
                            }
                        });
                        step.item = step.item.filter(
                            (x: any) => x.check_display == 1
                        );
                        ...
                        this.activationService
                            ...
                            .subscribe((data: any) => {
                                if (data.result == EnumStatus.ok) {
                                    step.audio = data.data.data.filter(
                                        (r: any) => r.question_type == 'audio'
                                    );
                                    step.video = data.data.data.filter(
                                        (r: any) => r.question_type == 'video'
                                    );
                                    step.image = data.data.data.filter(
                                        (r: any) => r.question_type == 'image'
                                    );
                                }
                            });
                    } else {
    ...
    loadDataActivationStep(work: any, work_id: any): any {
        this.activationService
            .work_step_GetList(Helper.ProjectID(), work_id)
            .subscribe((data: any) => {
                if (data.result == EnumStatus.ok) {
                    work.step = data.data.data;
                    if (Helper.IsNull(work.step) != true) {
                        work.step.forEach((item: any) => {
                            this.loadDataActivationItem(
                                item,
                                item.work_id,
                                item.work_item_id,
                                item.step_type_form
    ...
    loadDataActivation(report_id: any) {
        this.activationService
            .Works_GetList(Helper.ProjectID(), report_id)
            .subscribe((data: any) => {
                if (data.result == EnumStatus.ok) {
                    this.inValue.data_activation = data.data.data;
                    this.inValue.data_activation.report_id = report_id;

                    if (Helper.IsNull(this.inValue.data_activation) != true) {
                        this.inValue.data_activation.forEach((element: any) => {
                            element.step = data.data.dataStep.filter(
                                (st: any) =>
                                    st.work_id == element.work_id &&
                                    st.step_type_form == 'SURVEY'
                            );
                            element.stepGift = data.data.dataStep.filter(
                                (st: any) =>
                                    st.work_id == element.work_id &&
                                    st.step_type_form == 'GIFT'
                            )[0];

                            //haha
                            element.itemGift = data.data.itemGift.filter(
                                (x: any) =>
                                    x.work_id == element.work_id &&
                                    x.typeOf != 'alert' &&
                                    x.typeOf != 'final'
                            );
                            element.itemGift.forEach((g: any) => {
                                g.his =
                                    Helper.IsNull(g.history) != true
                                        ? JSON.parse(g.history)
                                        : [];
                            });
                            element.itemProduct = data.data.itemProduct.filter(
                                (x: any) =>
                                    x.work_id == element.work_id &&
                                    x.typeOf != 'alert' &&
                                    x.typeOf != 'final'
                            );
                            element.audio = data.data.itemFile.filter(
                                (r: any) =>
                                    r.work_id == element.work_id &&
                                    r.question_type == 'audio'
                            );
                            element.video = data.data.itemFile.filter(
                                (r: any) =>
                                    r.work_id == element.work_id &&
                                    r.question_type == 'video'
                            );
                            element.giftListWeb = element.itemGift.filter(
                                (x: any) => {
                                    return (
                                        x.employee_code != element.employee_code
                                    );
                                }
                            );
                            element.giftListApp = element.itemGift.filter(
                                (x: any) => {
                                    return (
                                        x.employee_code == element.employee_code
                                    );
                            ...
                            // element.image = data.data.itemFile.filter((r: any) => r.work_id == element.work_id && r.question_type == 'image');

                            if (Helper.IsNull(element.step) != true) {
                                element.step.forEach((st: any) => {
                                    if (st.step_type_form == 'SURVEY') {
                                        if (
                                            Helper.IsNull(
                                                data.data.itemSurvey
                                            ) != true
                                        ) {
                                            if (
                                                Helper.IsNull(
                                                ...
                                                st.list_question = JSON.parse(
                                                    st.list_question
                                                );
                                            }
                                            st.item =
                                                data.data.itemSurvey.filter(
                                                    (x: any) =>
                                                        x.work_id ==
                                                            st.work_id &&
                                                        x.work_item_id ==
                                            ...
                                            st.item = st.item.map((x: any) => ({
                                                ...x,
                                                _value_string: x.value_string,
                                                toolTip: `Create By: [${x.created_by}] - ${x.employee_code} - ${x.employee_name} | Create Date: ${x.created_date}`,
                                            }));
                                        }
                                        if (Helper.IsNull(st.item) != true) {
                                            st.item.forEach((i: any) => {
                                                if (
                                                    i.question_type ==
                                                        'multi-select' &&
                                                    i.check_display == 1
                                        ...
                                        if (Helper.IsNull(st.item) != true) {
                                            st.item = st.item.filter(
                                                (x: any) => x.check_display == 1
                                            );
                                        }

                                        // st.forEach((i: any) => {
                                        st.audio = data.data.itemFile.filter(
                                            (r: any) =>
                                                r.work_id == st.work_id &&
                                                r.work_item_id ==
                                                    st.work_item_id &&
                                                r.question_type == 'audio'
                                        );
                                        st.video = data.data.itemFile.filter(
                                            (r: any) =>
                                                r.work_id == st.work_id &&
                                                r.work_item_id ==
                                                    st.work_item_id &&
                                        ...
                                        st.image = data.data.itemFile.filter(
                                            (r: any) =>
                                                r.work_id == st.work_id &&
                                                r.work_item_id ==
                                                    st.work_item_id &&
                                                r.question_type == 'image'
                                        );

                                        // });
                                    } else {
                                        if (st.step_type_form == 'GIFT') {
                                            st.itemGift =
                                                data.data.itemGift.filter(
                                                    (x: any) =>
                                                        x.work_id ==
                                                            st.work_id &&
                                                        x.work_item_id ==
                                            ...
                                            st.step_type_form == 'SELLOUT'
                                        ) {
                                            st.itemProduct =
                                                data.data.itemProduct.filter(
                                                    (x: any) =>
                                                        x.work_id ==
                                                            st.work_id &&
                                                        x.work_item_id ==
                    ...
                    try {
                        const editData = this.dataQC.filter(
                            (qc: any) => qc.KPI == 'ACTIVATION'
                        )[0].is_edit_data;
                        this.inValue.data_activation.is_edit_data = editData;
                    } catch (error) {}
                }
            });
    }

    dataloadDataActivation(report_id: any) {
        this.loadDataActivation(report_id);
    }

    dataloadDataInventory(report_id: any) {
        this.loadDataInventory(this.project_id, report_id);
    }

    dataloadDataSOS(report_id: any) {
        this.loadDataSOS(this.project_id, report_id);
    }
        ...
        this.oolService
            ...
            .subscribe((data: any) => {
                if (data.result == EnumStatus.ok) {
                    ...
                    this.inValue.data_ool.details_item.forEach((e: any) => {
                        e.tooltip =
                            e.updated_code != null
                                ? 'Updated by: ' +
                                  e.updated_name +
        ...
        this.promotionService
            ...
            .subscribe((data: any) => {
                if (data.result == EnumStatus.ok) {
                    ...
                    this.inValue.data_promotion.detail.forEach((e: any) => {
                        e.tooltip =
                            e.update_by != null
                                ? 'Updated by: ' +
                                  e.updated_name +
                    ...
                    this.inValue.data_promotion.result.forEach(
                        (element: any) => {
                            ...
                            const guidItemSurvey = [] as any;
                            const guidItemProduct = [] as any;

                            const sortSurvey = [] as any;
                            element.detail.survey.forEach(
                                (x: any, index: any) => {
                                    guidItemSurvey.push(x.Guid_item);
                                    x.representative = {
                                        name: x.survey_name,
                            ...
                            element.detail.product.forEach((x: any) => {
                                guidItemProduct.push(x.Guid_item);
                                x.representative = {
                                    name:
                            ...
                            const _guidItemSurvey =
                                this.getDistinct(guidItemSurvey);
                            const _guidItemProduct =
                                this.getDistinct(guidItemProduct);

                            const resultGuiItemSurvey = [] as any;
                            _guidItemSurvey.forEach((gui: any) => {
                                resultGuiItemSurvey.push(
                                    element.detail.survey.filter(
                                        (x: any) =>
                                            x.Guid_item.toUpperCase() ==
                                            gui.toUpperCase()
                                    )
                                );
                            });
                            const resultGuiItemProduct = [] as any;
                            _guidItemProduct.forEach((gui: any) => {
                                resultGuiItemProduct.push(
                                    element.detail.product.filter(
                                        (x: any) =>
                                            x.Guid_item.toUpperCase() ==
                                            gui.toUpperCase()
                                    )
                            ...
                            element.detail._guidItemSurvey = _guidItemSurvey;
                            element.detail._guidItemNameSurvey =
                                resultGuiItemSurvey;
                            element.detail._guidItemProduct = _guidItemProduct;
                            element.detail._guidItemNameProduct =
                                resultGuiItemProduct;
                        }
                    );
        ...
        this.sellOutService
            ...
            .subscribe((data: any) => {
                if (data.result == EnumStatus.ok) {
                    ...
                    this.inValue.data_sellOut.details.forEach((e: any) => {
                        e.tooltip =
                            e.update_by != null
                                ? 'Updated by: ' +
                                  e.update_name +
        ...
        this.sellInService
            ...
            .subscribe((data: any) => {
                if (data.result == EnumStatus.ok) {
                    ...
                    this.inValue.data_sellIn.details.forEach((e: any) => {
                        e.tooltip =
                            e.update_by != null
                                ? 'Updated by: ' +
                                  e.update_name +
    ...
    deleteRequest(item: any) {
        this.confirmationService.confirm({
            message: 'Bạn có muốn xóa yêu cầu này ?',
            header: 'Xóa yêu cầu giải trình',
    ...
    ngOnInit(): void {
        ...
        this._service.imageEmp = this.inValue.images;
    ...
    reloadDataATD(event: any) {
        this.loadDataATD(Helper.ProjectID(), this.inValue.report_id);
    }
    ...
    readDataActivation(event: any) {
        this.loadDataActivation(event);
    }
    ...
    // unique(arr: any) {
    //     var newArr: any = []
    //     let tempArr = arr.map((item: any) => item.r);

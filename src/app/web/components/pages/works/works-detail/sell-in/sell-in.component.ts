...
export class SellInComponent {
    ...
    ngOnInit(): void {
        ...
        this.projectName();
        let tmp = this.inValue.data_sellIn.result || [];

        this.products = [];
        this.categorys = [];
        tmp.forEach((e: any) => {
            e.details.forEach((d: any) => {
                this.products.push(d._product);
                this.categorys.push(d._category);
            });
        });
        if (this.products.length > 0)
            this.getDistinctObjects(this.products, '_product');
        if (this.categorys.length > 0)
            this.getDistinctObjects(this.categorys, '_category');
        this.itemsButton = [
            {
                label: 'Update',
                icon: 'pi pi-refresh',
    ...
    selectImageType(event: any) {
        ...
        if (Helper.IsNull(this.selectedImageType) != true) {
            this.checkImageType =
                this.selectedImageType.code == 'PRODUCT' ? true : false;
        } else {
    ...
    saveImage() {
        if (
            this.NofiIsNull(this.selectedImageType, 'image type') == 1 ||
            (this.checkImageType == true &&
            ...
            this._service
                .SellIn_Image_Action(
                    Helper.ProjectID(),
                    this.listTab.sellin_id,
                    this.checkImageType == true
                        ? this.selectedProduct.product_id
                        : null,
                    0,
                    this.selectedImageType.code,
                    this.url,
                    'create'
                )
                .subscribe((data: any) => {

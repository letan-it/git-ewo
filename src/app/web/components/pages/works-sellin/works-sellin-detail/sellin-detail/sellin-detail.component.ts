...
@Component({
  selector: 'app-sellin-detail',
  templateUrl: './sellin-detail.component.html',
  styleUrls: ['./sellin-detail.component.scss']
})
export class SellinDetailComponent {
  ...
  onRowEditSave(item: any, tab: any) {
    if (this.checkValue(item.quantity, 'quantity') == 1 ||
      this.checkValue(item.price, 'price') == 1) {
      return
    } else {

      this._service.SellIn_Detail_Action(Helper.ProjectID(), item.id, item.quantity, item.price)
        .subscribe((data: any) => {
          if (data.result == EnumStatus.ok) {
            this.NofiResult('Page Works', 'Update product details',
              `Update information successfully (quantity: ${item.quantity} and price: ${item.price}) product details ${item._product}`, 'success', 'SuccessFull')
            item.total = this.total(item.quantity, item.price)
            let sumTotal = 0
            this.inValue.data_sellIn.result.forEach((r: any) => {
              console.log('r.sellin_id == tab.sellin_id : ', r.sellin_id, tab.sellin_id)
              console.log('r.UUID == tab.UUID : ', r.UUID, tab.UUID)
              console.log('r.Guid == tab.Guid : ', r.Guid, tab.Guid)

              if (r.sellin_id == tab.sellin_id && r.UUID == tab.UUID && r.Guid == tab.Guid) {
                ...
                console.log('SellIn Details listTotal : ', listTotal)
              }
              console.log('SellIn Details listTotal2 : ')
            })
            console.log('SellIn Details listTotal3 : ')
            this.loadData(tab)
          }
        })
    }
  ...
  deleteImageAccept(tab: any) {
    ...
    this._service.SellIn_Image_Action(Helper.ProjectID(), tab.sellin_id, 0, this.imageDelete._idDetails, this.imageDelete.image_type, this.imageDelete.src, 'delete')
      .subscribe((data: any) => {
        if (data.result = EnumStatus.ok) {
          this.inValue.data_sellIn.result.forEach((r: any) => {
            if (r.sellin_id == tab.sellin_id && r.UUID == tab.UUID && r.Guid == tab.Guid) {
              r.image.listphotoImage = r.image.listphotoImage.filter((x: any) => x.id != this.imageDelete.id)
            }
          })
          this.NofiResult('Page Works', 'Delete images', 'Delete successful images', 'success', 'SuccessFull')
          this.loadData(tab)
        }
      })
  }
  ...
  selectProductData(event: any) {
    this.selectedProduct = (event != null && event.value != null) ? event.value : null;
  }

  selectedImageType: any = null
  checkImageType: boolean = true;
  selectImageType(event: any) {
    this.selectedImageType = (event != null && event.value != null) ? event.value : null;
    if (Helper.IsNull(this.selectedImageType) != true) {
      this.checkImageType = this.selectedImageType.code == 'PRODUCT' ? true : false
    } else {
  ...
  saveImage() {
    ...
    if (this.NofiIsNull(this.selectedImageType, 'image type') == 1 ||
      (this.checkImageType == true && this.NofiIsNull(this.selectedProduct, 'product id') == 1) ||
      this.NofiIsNull(this.url, 'image') == 1) {
      return;
    } else {
      this._service.SellIn_Image_Action(Helper.ProjectID(), this.listTab.sellin_id, this.checkImageType == true ? this.selectedProduct.product_id : null, 0,
        this.selectedImageType.code, this.url, 'create')
        .subscribe((data: any) => {
          if (data.result == EnumStatus.ok) {
            this.NofiResult('Page Work', 'More photos', 'Add successful images', 'success', 'SuccessFull')
            this.hideDialog()
  ...
  chooseConfirm(item: any, tab: any) {
    ...
    if (item.disible == 0) {
      this._service.SellIn_Result_Action(Helper.ProjectID(), tab.sellin_id, item.Id, tab.comfirm_note)
        .subscribe((data: any) => {
          if (data.result == EnumStatus.ok) {
            this.NofiResult('Page Work', 'Confirm the status of the report', `Confirm the status of successful reporting (Note: ${tab.comfirm_note})`, 'success', 'SuccessFull');
            this.loadData(tab)
          }
        })

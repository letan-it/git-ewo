...
export class PromotionShopComponent implements OnInit {
    constructor(
    ...
    action() {
        if (
            ...
            // this.checkNumber(this.target, 'input') == 1 ||
            // this.checkStatus(this.target, 'Input must be -1 or 0 or 1') == 1
        ) {
            return;
        } else {
            this._service
                ...
                .subscribe((res: any) => {
                    if (res.result == EnumStatus.ok) {
                        this.NofiResult(
                            'Page promotion',
                            'Update promotion shop',
                            `Successfully edited the status = ${this.status} of the promotion code
          (${this.promotion_code}) for shops code (${this.shop_code})`,
                            'success',
                            'Successfull'
                        );
                        this.handleFilter(1);
    ...
    viewPromotionShop() {
        this.router.navigate(['/activation/promotion']);
    }

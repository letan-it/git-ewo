...
export class PromotionDetailsComponent {
  ...
  ngOnInit(): void {
    ...
    // this.colSpanGroup = 4
    try {
      this.selectedColumns = Helper.IsNull(JSON.parse(localStorage.getItem('selectedColumns') + '')) != true ?
        JSON.parse(localStorage.getItem('selectedColumns') + '') : []
      this.colSpanGroup = Helper.IsNull(JSON.parse(localStorage.getItem('colSpanGroup') + '')) != true
        ? JSON.parse(localStorage.getItem('colSpanGroup') + '') : 4
  ...
  onChangeColumns(event: any) {
    this.colSpanGroup = 4
    this.colSpanGroup = event != null ? (this.colSpanGroup + event?.value.length) : this.colSpanGroup
  ...
  onChangeQuestion(event: any, item: any, checkValue: any) {
    ...
    if (checkValue == 4) {
      item.value_datetime = this.transformValue(item.support_data, item.question_type, item.typeOf, item.value_string)
    }
  }
  ...
  editDataQuestion1(listQuestionEdit: any) {
    ...
    listQuestionEdit.filter((x: any) => x.id != 139 && x.id != 140 && x.id != 141)
      .forEach((element: any) => {
        const checkType = this.checkValue(element.support_data, element.question_type, element.typeOf)

        if (checkType == 8 && Helper.IsNull(element.value_string) != true && element.value_string.length > 0) {
    ...
    if (result && result.length > 0) {
      this._service.Promotion_result_detail_Update(listQuestionEdit.pr_id, listQuestionEdit.pr_item_id, result)
        .subscribe((data: any) => {
          if (data.result == EnumStatus.ok) {
            this.NofiResult('Page Work', 'Update Question', 'Update Successful Questions', 'success', 'SuccessFull');
            this.outValue.emit(this.tab.report_id);
            this.clearDataQuestion()
          } else {
            this.NofiResult('Page Work', 'Update Question', 'Update Error Questions', 'error', 'Error');
            this.clearDataQuestion()
          }
        })
    } else {
  ...
  editDataQuestion(listQuestionEdit: any) {
    ...
    listQuestionEdit.filter((x: any) => x.id != 139 && x.id != 140 && x.id != 141)
      .forEach((element: any) => {
        ...
        result.push({
          ...
          value_string: checkType != 8 ? element.value_string : element._listAnswer,
    ...
    // ?report_id 
    if (result && result.length > 0) {
      this._service.Promotion_result_UpdateQuestion(Helper.ProjectID(), this.tab.year_month, result)
        .subscribe((data: any) => {
          if (data.result == EnumStatus.ok) {
            this.NofiResult('Page Work', 'Update Question', 'Update Successful Questions', 'success', 'SuccessFull');
            this.outValue.emit(this.tab.report_id);
  ...
  addImageDetails() {
    ...
    if (this.NofiIsNull(this.question_name, 'question name') == 1 ||
      ...
      this._service.Promotion_result_detail_file_Action
        (Helper.ProjectID(),
          0,
          ...
          'create').subscribe((data: any) => {
            if (data.result == EnumStatus.ok) {
              ...
              this.inValue.data_promotion.result.forEach((data: any) => {
                if (this.item_data == 'PRODUCT' && data.pr_id == this.item.pr_id) {
                  const i = data.image.product.listphotoImage.length;
                  data.image.product.listphotoImage.push({
                    id: i + 1,
                    _id: 0, // List ?
                    pr_item_id: this.item.pr_item_id,
                    pr_id: this.item.pr_id,
                    question_id: this.question_name.question_id,
                    src: this.url,
                    title: `${this.question_name.question_name}`,
                    question_name: this.question_name.question_name,
                    product: `${this.item.product_code} - ${this.item.product_name}`,
                    image_time: dateTime,
                    _index: i + 1
                  })
                } else if (this.item_data == 'SURVEY' && data.pr_id == this.item.pr_id) {
                  const i = data.image.survey.listphotoImage.length;
              ...
              this.NofiResult(`Page Work`, `More photos`, this.item_data == `PRODUCT` ?
                `Add successful images for question ${this.question_name.question_name} of a product with a code of ${this.item.product_code}`
                : `Add successful images for question ${this.question_name.question_name} of a survey with a name of ${this.item.survey_name}`, `success`, `Successfull`);
  ...
  deleteImageAccept() {
    this._service.Promotion_result_detail_file_Action(
      ...
      if (data.result == EnumStatus.ok) {
        this.NofiResult(`Page Works`, `Delete images`, this.item_data == `PRODUCT` ?
          `Successfully deleting the image of question ${this.imageDelete.question_name} of the product ${this.imageDelete.product}`
          : `Successfully deleting the image of question ${this.imageDelete.question_name} of the survey ${this.imageDelete.survey}`, `success`, `Successfull`);

        this.inValue.data_promotion.result.forEach((data: any) => {
          this.item_data == `PRODUCT` && data.pr_id == this.imageDelete.pr_id ?
            data.image.product.listphotoImage = data.image.product.listphotoImage.filter((x: any) => x._id != this.imageDelete._id)
            : data.image.survey.listphotoImage = data.image.survey.listphotoImage.filter((x: any) => x._id != this.imageDelete._id)
        })
  ...
  checkValueInt(support_data: any, question_type: any, typeOf: any, value_int: any, value_string: any): any {
    const checkValue = this.checkValue(support_data, question_type, typeOf);
    //return (checkValue == 1 || checkValue == 3 || checkValue == 4 || checkValue == 5) ? null : value_int
    if (checkValue == 1 || checkValue == 3 || checkValue == 4 || checkValue == 5) {
      return null
  ...
  actionLocation(event: any, item: any, key: any) {
    this.inValue.data_promotion.result.forEach((l: any) => {
      ...
      if (item.item_data == 'SURVEY') {
        this.tabDetailsData = Helper.IsNull(l.detail.survey) != true ? l.detail.survey : []
      } else {
        this.tabDetailsData = Helper.IsNull(l.detail.product) != true ? l.detail.product : []
      }
    });

    const ListLocation = this.tabDetailsData?.filter((s: any) =>
      (s.support_data == 'Provinces' || s.support_data == 'Districts' || s.support_data == 'Wards') && s.question_type == 'text' && s.typeOf == 'int')

    const result = ListLocation.filter((x: any) => x.pr_item_id == item.pr_item_id && x.pr_id == item.pr_id && x.item_data == item.item_data
      && x.data_form_survey == item.data_form_survey && x.product_id == item.product_id)
  ...
  transformValue(support_data: any, question_type: any, typeOf: any, value_string: any): any {
    // support_data, question_type, typeOf

    if (support_data == 'date' && question_type == 'text' && typeOf == 'string') {
      return this.datePipe.transform(new Date(value_string), "yyyy-MM-dd");
    } else if (support_data == 'datetime' && question_type == 'text' && typeOf == 'string') {
      return this.datePipe.transform(new Date(value_string), "yyyy-MM-dd HH:mm:ss");
    } else {
      return value_string;
    }
  }
  ...
  onRowEditSave(item: any, tab: any) {
    ...
    const checkType = this.checkValue(item.support_data, item.question_type, item.typeOf)
    if (checkType != 8 && this.NofiIsNull(item.value_string, 'value') == 1 ||
      checkType == 3 && this.NofiDecimal(item.value_string, 'Value') == 1 ||
      checkType == 2 && this.checkNumber(parseInt(item.value_string), 'Value') == 1) {
      return;
    } else {
      ...
      } else if (checkType == 8 && Helper.IsNull(item.value_string) != true) {
        this.actionListAnswer(item.value_string, item)

      } else {
        item.value_string = checkType == 2 || checkType == 3 ? item.value_string + `` : item.value_string
      }

      item.value_int = this.checkValueInt(item.support_data, item.question_type, item.typeOf, item.value_int, item.value_string);
      item.value_string = this.transformValue(item.support_data, item.question_type, item.typeOf, item.value_string)

      if (checkType == 9 || checkType == 10 || checkType == 11) {
        if (this.NofiIsNull(item.item_Province, 'province') == 1 ||
          this.NofiIsNull(item.item_District, 'district') == 1 ||
          this.NofiIsNull(item.item_Ward, 'ward') == 1) {
          return;
        } else {
          this.updateDetailsLocation(checkType, tab, item, item._idProvince, item.item_Province, item._noteProvince);
          this.updateDetailsLocation(checkType, tab, item, item._idDistrict, item.item_District, item._noteDistrict);
          this.updateDetailsLocation(checkType, tab, item, item._idWard, item.item_Ward, item._noteWard);
        }
      } else if (checkType != 9 && checkType != 10 && checkType != 11) {
  ...
  updateDetailsLocation(checkType: any, tab: any, item: any, id: any, value: any, note: any) {
    item.id = id;
  ...
  updateDetails(checkType: any, tab: any, item: any) {
    this._service.Promotion_result_detail_Action(
      Helper.ProjectID(), item.id,
      item.value_int,
      ...
      .subscribe((data: any) => {
        ...
        if (data.result == EnumStatus.ok) {
          ...
          if (checkType != 9 && checkType != 10 && checkType != 11) {
            this.NofiResult(`Page Works`, `Edit question information`, `Successfully editing information of question ${item.question_name} 
              (Value : ${this.transformValue(item.support_data, item.question_type, item.typeOf,
              checkType != 8 ? `${item.value_string} ,` : item.value_string_info)} Note : ${item.note})`,
              `success`, `Successfull`)
          } else {
            this.NofiResult(`Page Works`, `Edit question information`, `Successful update of information province - district - ward `, `success`, `Successfull`)
          }
          if (checkType == 8) {
  ...
  callCalculatorResult(tab: any) {
    if (Helper.IsNull(tab.formula_file_js) != true) {
      this._service.Promotion_result_detail_JsonResult(Helper.ProjectID(), tab.pr_id)
        .subscribe((data: any) => {
          if (data.result == EnumStatus.ok) {
            try {
              const jsonResult = data.data[0].dataJson
              this.scriptLoader(tab.formula_file_js, jsonResult, tab)
  ...
  callCalculatorFunction(value: any, tab: any): void {
    ...
    try {
      ...
      const dataJson = Helper.IsNull(data) != null ? JSON.parse(data) : null
      if (Helper.IsNull(dataJson) != true) {
        const result = dataJson.result;
        const result_note = dataJson.note

        if (Helper.IsNull(data) != null && (result == 1 || result == 0) && Helper.IsNull(result_note) != true) {
          this._service.Promotion_result_UpdateResult(Helper.ProjectID(), tab.pr_id, result, result_note, data)
            .subscribe((data: any) => {
              if (data.result == EnumStatus.ok) {
                // this.outValue.emit(tab.report_id);
                tab.tagResult = (result == 1) ? 'Đạt' : 'Rớt'
                console.log('tagResult : ', tab.tagResult)
                this.Nofi('success', 'SuccessFull', `Successful update results (${tab.tagResult}) of the promotion report`)
              }
            })
        } else {
          return
  ...
  scriptLoader(scriptUrl: any, jsonResult: any, tab: any) {
    ...
    this.scriptLoaderService.loadScript(scriptUrl).then(() => {
  ...
  editQuestion(item: any, tab: any, tabdetails: any, item_data: any) {
    ...
    item.headerName = (item_data == 'PRODUCT') ? `PRODUCT : ${item.product_code} - ${item.product_name}` : `SURVEY : ${item.survey_name}`

    this.surveyService.Survey_question_FormEdit(item.data_form_survey)
      .subscribe((data: any) => {
        if (data.result == EnumStatus.ok) {

          this.listQuestionEdit = data.data
          const dataTab = tabdetails.filter((x: any) => x.product_id == item.product_id)

          this.listQuestionEdit.forEach((element: any) => {
            try {
              this.listQuestionEdit.pr_item_id = this.getData(dataTab, element.question_id).pr_item_id
              this.listQuestionEdit.pr_id = this.getData(dataTab, element.question_id).pr_id

              // element.id = Helper.IsNull(this.getData(dataTab, element.question_id).id) != true ? this.getData(dataTab, element.question_id).id : 0
              element.pr_item_id = this.getData(dataTab, element.question_id).pr_item_id
              element.pr_id = this.getData(dataTab, element.question_id).pr_id
              element.item_data = this.getData(dataTab, element.question_id).item_data
              element.data_form_survey = this.getData(dataTab, element.question_id).data_form_survey
              element.product_id = this.getData(dataTab, element.question_id).product_id

              element.answer = JSON.parse(element.answer)
              element.value_datetime = this.getData(dataTab, element.question_id).value_datetime
              element.value_decimal = this.getData(dataTab, element.question_id).value_decimal
              element.value_int = this.getData(dataTab, element.question_id).value_int
              element.value_string = this.getData(dataTab, element.question_id).value_string
              element.note = this.getData(dataTab, element.question_id).note
              element.answer_option = this.getData(dataTab, element.question_id).answer_option
              element._value_string = this.getData(dataTab, element.question_id)._value_string
              element._listAnswer = this.getData(dataTab, element.question_id)._listAnswer

              element.item_Province = this.getData(dataTab, element.question_id).item_Province
              element.item_District = this.getData(dataTab, element.question_id).item_District
              element.item_Ward = this.getData(dataTab, element.question_id).item_Ward

              element._noteProvince = this.getData(dataTab, element.question_id)._noteProvince
              element._noteDistrict = this.getData(dataTab, element.question_id)._noteDistrict
              element._noteWard = this.getData(dataTab, element.question_id)._noteWard
  ...
  calculateCustomerTotal(name: string, list: any) {
    ...
    if (list) {
      for (let item of list) {
        if (item.representative?.name === name) {
          total++;
        }
      }
    }

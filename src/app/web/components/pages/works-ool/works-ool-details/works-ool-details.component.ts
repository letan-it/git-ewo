...
@Component({
  selector: 'app-works-ool-details',
  templateUrl: './works-ool-details.component.html',
  styleUrls: ['./works-ool-details.component.scss'],
  providers: [ConfirmationService, DatePipe, MessageService]
})
export class WorksOolDetailsComponent implements OnInit {
  constructor(
  ...
  loadItemKPI() {
    ...
    try {
      this._service.ewo_ShopCheckResult_GetList(Helper.ProjectID(), this.inValue.report_id)
        .subscribe((data: any) => {
          this.is_loadForm = 0;
          ...
          if (data.result == EnumStatus.ok) {
            ...
            const listImage = data.data.filter((x: any) => x.key_check == 'image')
            const list = data.data.filter((y: any) => y.key_check != 'image' && y.key_check != 'latitude' && y.key_check != 'longitude')

            const listphoto = [];
            const listphotoImage = [];

            if (Helper.IsNull(listImage) != true && listImage.length > 0) {
              listphoto.push({
                id: 1,
                src: listImage[0].data_check,
                result: listImage[0].result,
                title: 'Ảnh tổng quan' + ': ' + listImage[0].app_time,
                image_time: listImage[0].app_time,
                _index: 1
              })
            }
            if (Helper.IsNull(listImage) != true && listImage.length > 1) {
              for (var i = 0; i < listImage.length; i++) {
                listphotoImage.push({
                  id: i + 1,
                  src: listImage[i].value_string,
                  title: 'Ảnh mới' + ': ' + listImage[i].app_time,
            ...
            const dataLatitude = data.data.filter((x: any) => x.key_check == 'latitude')[0];
            const dataLongitude = data.data.filter((x: any) => x.key_check == 'longitude')[0];

            if (Helper.IsNull(dataLatitude) != true && Helper.IsNull(dataLongitude) != true) {
              const data_checklatitude = dataLatitude.data_check;
              const resultlatitude = dataLatitude.result;
              const value_stringlatitude = dataLatitude.value_string;

              const data_checklongitude = dataLongitude.data_check;
              const resultlongitude = dataLongitude.result;
              const value_stringlongitude = dataLongitude.value_string;

              // Tính khoảng cách
              var distance = Pf.haversine(data_checklatitude, data_checklongitude, value_stringlatitude, value_stringlongitude);
              // In ra khoảng cách 

              this.listDataLocation.push({
                id: 1,
                src: this.linkMap(data_checklatitude, data_checklongitude),
                data_check: data_checklatitude,
                result: resultlatitude,
                value_string: value_stringlatitude,
                haversine: distance.toFixed(2),
                title: 'Ảnh tọa độ' + ': ' + dataLatitude.app_time,
              ...
              this.listDataLocation.push({
                id: 2,
                src: this.linkMap(value_stringlatitude, value_stringlongitude),
                data_check: data_checklongitude,
                result: resultlongitude,
                value_string: value_stringlongitude,
                title: 'Ảnh tọa độ' + ': ' + dataLongitude.app_time,
            ...
            this.inValue.data_checkShop.listphotoImage = listphotoImage
            this.inValue.data_checkShop.listLocation = this.listDataLocation
        ...
        .subscribe((data: any) => {
          if (data.result == EnumStatus.ok) {
            ...
            if (data.data.length > 0) {
              ...
              this.listphoto = [];
              for (var i = 0; i < this.ListATT_getItem.length; i++) {
                this.listphoto.push({
                  id: i + 1,
                  src: this.ListATT_getItem[i].atd_photo,
                  title: this.ListATT_getItem[i].atd_type + ': ' + this.ListATT_getItem[i].atd_time,
                  image_time: this.ListATT_getItem[i].atd_time,
                  _index: i + 1
                })
              }
          ...
          if (data.result == EnumStatus.ok) {
            ...
            if (data.data.result.length > 0) {
              ...
              if (this.FilterSurvey > 0) {
                this.ListSurveyResult = this.ListSurveyResult.filter((item: any) => item.survey_id == this.FilterSurvey);
      ...
      this.osaService
        ...
        .subscribe((data: any) => {
          if (data.result == EnumStatus.ok) {
            ...
            this.inValue.data_osa.osA_Results.forEach((element: any) => {
              element.detail = this.inValue.data_osa.osA_Detail.filter((item: any) => item.Guid == element.Guid)
              ...
              element.detail.forEach((x: any, index: any) => {
                ...
                x.representative = {
                  ...
                  color: x.color == "" ? Helper.colorArray(index) : x.color
                };
              ...
              element.sort = _temp
              element.image = this.inValue.data_osa.osA_image.filter((item: any) => item.Guid == element.Guid)

              element.detail.fromOOC = element.detail.filter((x: any) => x.ooc != null).length > 0 || element.detail[0].check_ooc == 1 ? 1 : 0
              element.detail.fromOOS = element.detail.filter((x: any) => x.oos != null).length > 0 || element.detail[0].check_oos == 1 ? 1 : 0
              element.detail.fromQuantity = element.detail.filter((x: any) => x.quantity != null).length > 0 || element.detail[0].check_qty == 1 ? 1 : 0
              element.detail.fromPrice = element.detail.filter((x: any) => x.price != null).length > 0 || element.detail[0].check_price == 1 ? 1 : 0
              element.detail.fromFacing = element.detail.filter((x: any) => x.facing != null).length > 0 || element.detail[0].check_facing == 1 ? 1 : 0

            });

            try {
              const editData = this.dataQC.filter((qc: any) => qc.KPI == 'OSA')[0].is_edit_data;
  ...
  loadDataOOL(report_id: any) {
    ...
    this.oolService.OOL_Results_GetList(Helper.ProjectID(), report_id)
      .subscribe((data: any) => {
        if (data.result == EnumStatus.ok) {

          this.inValue.data_ool = data.data;

          if (Helper.IsNull(this.inValue.data_ool.details_item) != true) {
            this.inValue.data_ool.details_item = this.inValue.data_ool.details_item.map((x: any) => ({
              ...x,
              history: Helper.IsNull(x.history) != true ? JSON.parse(x.history) : [],
              ...
              answer_option: (Helper.IsNull(x.answer_option) != true && x.answer_option.length > 0) ? JSON.parse(x.answer_option) : [],
              product_option: (Helper.IsNull(this.listProduct) != true && this.listProduct.length > 0) ? this.listProduct : []
          ...
          if (Helper.IsNull(this.inValue.data_ool.details_item) != true) {
            this.inValue.data_ool.details_item.forEach((x: any) => {
              ...
              if (x.question_type == 'select' || x.question_type == 'multi-select') {
                x._value_string = (Helper.IsNull(x.answer_option) != true && x.answer_option.length > 0 && Helper.IsNull(x.value_int) != true) ? ((x.question_type == 'select') ?
                  x.answer_option.filter((element: any) => element.ool_answer_id == x.value_int)[0]
                  : x.answer_option.filter((element: any) => element.ool_answer_id == x.value_int)) : []

              } else if (x.support_data == 'select-product' || x.support_data == 'multi-select-product') {
                x._value_string = (Helper.IsNull(x.product_option) != true && x.product_option.length > 0 && Helper.IsNull(x.value_int) != true) ? ((x.support_data == 'select-product') ?
                  x.product_option.filter((element: any) => element.ool_answer_id == x.value_int)[0]
                  : x.product_option.filter((element: any) => element.ool_answer_id == x.value_int)) : []
              }

              if (Helper.IsNull(x.history) != true) {
                x.history.filter((c: any) => c.show_history != 1).forEach((h: any) => {

                  if (Helper.IsNull(h.delete_guild) != true) {
                    const _history = x.history.filter((x1: any) => x1.show_history == 1)
                    _history.forEach((dv: any) => {
                      if (dv.delete_guild == h.delete_guild) {
          ...
          const listMultiSelect = (Helper.IsNull(this.inValue.data_ool.details_item) != true) ?
            this.inValue.data_ool.details_item.filter((x: any) => x.support_data == null && x.question_type == 'multi-select' && x.typeOf == 'int') : []
          listMultiSelect.filter((x: any) => x.show_web == 1).forEach((element: any) => {
            this.loadValueString(listMultiSelect, element);
          });

          const listMultiSelect_Product = (Helper.IsNull(this.inValue.data_ool.details_item) != true) ?
            this.inValue.data_ool.details_item.filter((x: any) => x.support_data == 'multi-select-product' && x.question_type == 'text' && x.typeOf == 'int') : []
          listMultiSelect_Product.filter((x: any) => x.show_web == 1).forEach((element: any) => {
            this.loadValueString(listMultiSelect_Product, element);
          });
          ...
          if (Helper.IsNull(this.inValue.data_ool.details) != true) {
            this.inValue.data_ool.details.forEach((detail: any) => {
              ...
              detail.image = (Helper.IsNull(this.inValue.data_ool.image) != true) ?
                this.inValue.data_ool.image.filter((item: any) => item.ool_result_id == detail.ool_result_id && item.ool_detail_id == detail.ool_detail_id && item.UUID == detail.UUID) : [];
              detail.details_item = (Helper.IsNull(this.inValue.data_ool.details_item) != true) ?
                this.inValue.data_ool.details_item.filter((item: any) => item.ool_result_id == detail.ool_result_id && item.ool_detail_id == detail.ool_detail_id && item.UUID == detail.UUID) : [];
          ...
          this.inValue.data_ool.result.forEach((element: any) => {
            element.details = Helper.IsNull(this.inValue.data_ool.details) != true ?
              this.inValue.data_ool.details.filter((item: any) => item.ool_result_id == element.ool_result_id && item.UUID == element.UUID) : [];

          });

          try {

            const editData = this.dataQC.filter((qc: any) => qc.KPI == 'OOL')[0].is_edit_data;
            this.inValue.data_ool.is_edit_data = editData;

            this.inValue.data_ool.result.forEach(
              (element: any) => {
                try {
                  element.is_edit_data = editData;
                } catch (error) {
        ...
        console.log('this.inValue.data_ool : ', this.inValue.data_ool)
      })
  }
  ...
  loadValueString(listMultiSelect: any, element: any) {
    ...
    if (Helper.IsNull(listMultiSelect) != true) {
      listMultiSelect.filter((x: any) => x.ool_result_id == element.ool_result_id && x.ool_detail_id == element.ool_detail_id
        && x.ool_id == element.ool_id && x.layer == element.layer && x.ool_item_id == element.ool_item_id).forEach((z: any) => {
  ...
  getListProduct() {
    ...
    this.productService.ewo_Products_GetList(Helper.ProjectID(), '', '', 0, '', 1000000, 1)
      .subscribe((data: any) => {
        this.listProduct = []
        if (data.result == EnumStatus.ok) {
          if (data.data.length > 0) {
  ...
  listItemMaster: any = []
  loadMaster() {
    this.oolService.OOL_item_Master(Helper.ProjectID())
      .subscribe((data: any) => {
        if (data.result == EnumStatus.ok) {
          this.listItemMaster = []
          if (data.data.length > 0) {
  ...
  loadDataPromotion(report_id: any) {
    this.promotionService.Promotion_result_GetList(Helper.ProjectID(), report_id)
      .subscribe((data: any) => {
        ...
        if (data.result == EnumStatus.ok) {
          ...
          this.inValue.data_promotion.result.forEach((element: any) => {
            element.detail = this.inValue.data_promotion.detail.filter((item: any) => item.Guid == element.Guild);
            element.detail.forEach((element: any) => {
              element.answer_option = Helper.IsNull(element.answer_option) != true ? JSON.parse(element.answer_option) : []
              element.detail_item = Helper.IsNull(element.detail_item) != true ? JSON.parse(element.detail_item) : []

              if (element.answer_option.length > 0 && Helper.IsNull(element.value_int) != true) {
                element._value_string = element.answer_option.filter((x: any) => x.answer_id == element.value_int)[0]
            ...
            const surveyListAnswer = element.detail.filter((s: any) =>
              s.support_data == null && s.question_type == 'multi-select' && s.typeOf == 'int')
            surveyListAnswer.filter((x: any) => x.show_web == 1).forEach((element: any) => {

              element.value_string = []
              surveyListAnswer.filter((x: any) => x.pr_item_id == element.pr_item_id && x.pr_id == element.pr_id && x.item_data == element.item_data
                && x.data_form_survey == element.data_form_survey && x.product_id == element.product_id && x.question_id == element.question_id).forEach((z: any) => {
                  element.value_string.push(z._value_string)
                });

              element._listAnswer = ''
              element.value_string_info = ''
              element.value_string?.forEach((x: any) => {
                if (Helper.IsNull(x.answer_id) != true) {
            ...
            const surveyListLocation = element.detail.filter((s: any) =>
              (s.support_data == 'Provinces' || s.support_data == 'Districts' || s.support_data == 'Wards') && s.question_type == 'text' && s.typeOf == 'int')
            ...
            surveyListLocation.forEach((element: any) => {
              ...
              surveyListLocation.filter((x: any) => x.pr_item_id == element.pr_item_id && x.pr_id == element.pr_id && x.item_data == element.item_data
                && x.data_form_survey == element.data_form_survey && x.product_id == element.product_id).forEach((z: any) => {

                  if (z.support_data == 'Districts' && element.support_data == 'Provinces') {

                    z.item_District = {
                      name: z.value_string,
                      code: z.value_int
                    }
                    element.item_District = z.item_District
                    z.item_Province = element.item_Province
                    ...
                    element._noteProvince = element.note
                    element._noteDistrict = z.note

                  }
                  else if (z.support_data == 'Wards' && element.support_data == 'Provinces') {
                    ...
                    z._noteProvince = element._noteProvince
                    z._noteDistrict = element._noteDistrict
                    z._noteWard = z.note
                    element._noteWard = z.note

                  } else if (z.support_data == 'Wards' && element.support_data == 'Districts') {
                    ...
                    element.item_Ward = z.item_Ward
                    z.item_District = element.item_District
            ...
            element.image = this.inValue.data_promotion.image.filter((item: any) => item.Guid == element.Guild && item.question_type != 'audio');
            element.audio = this.inValue.data_promotion.image.filter((item: any) => item.Guid == element.Guild && item.question_type == 'audio');
          ...
          this.inValue.data_promotion.result.forEach((element: any) => {
            ...
            element.detail.forEach((d: any) => {
              d.history = Helper.IsNull(d.history) != true ? JSON.parse(d.history) : [];

              d.history.filter((c: any) => c.show_history != 1).forEach((h: any) => {

                if (Helper.IsNull(h.delete_guild) != true) {
                  const _history = d.history.filter((x: any) => x.show_history == 1)
                  _history.forEach((x: any) => {
                    if (x.delete_guild == h.delete_guild)
            ...
            element.detail.survey = element.detail.filter((item: any) => item.item_data == 'SURVEY');
            element.detail.product = element.detail.filter((item: any) => item.item_data == 'PRODUCT');
            element.image.survey = element.image.filter((item: any) => item.item_data == 'SURVEY');
            element.image.product = element.image.filter((item: any) => item.item_data == 'PRODUCT');
            element.audio.survey = element.audio.filter((item: any) => item.item_data == 'SURVEY');
            element.audio.product = element.audio.filter((item: any) => item.item_data == 'PRODUCT');

            const sortSurvey = [] as any
            element.detail.survey.forEach((x: any, index: any) => {
              x.representative = {
                name: x.survey_name,
              ...
              x.question = this.inValue.data_promotion.question.filter((y: any) =>
                y.data_form_survey == x.data_form_survey && element.promotion_id == y.promotion_id && y.item_data == 'SURVEY')

            });
            element.detail.productList = []
            const _temp = this.getDistinctObjects(
              element.detail.product, 'product_id'
            );
            ...
            element.detail.product.forEach((x: any) => {
              ...
              x.representative = {
                name: x.product_code + ' - ' + x.product_name,
                value: x.product_code + ' - ' + x.product_name,

                image: x.product_image,
                color: element.detail.productList.filter((y: any) => y.product_id == x.product_id)[0].color
              };
              ...
              x.question = this.inValue.data_promotion.question.filter((y: any) =>
                y.data_form_survey == x.data_form_survey && y.product_id == x.product_id
                && element.promotion_id == y.promotion_id && y.item_data == 'PRODUCT')
            })
            const photoImageSurvey = []
            const listImageSurvey = (element.image as any == [] as any) ? [] : element.image.survey;
            // pr_id: 2
            if (Helper.IsNull(listImageSurvey) != true && listImageSurvey.length > 0) {
              for (var i = 0; i < listImageSurvey.length; i++) {
                photoImageSurvey.push({
                  id: i + 1,
                  _id: listImageSurvey[i].id,
                  pr_item_id: listImageSurvey[i].pr_item_id,
                  pr_id: listImageSurvey[i].pr_id,
                  question_id: listImageSurvey[i].question_id,
                  src: listImageSurvey[i].url,
                  title: `${listImageSurvey[i].question_name} - ${listImageSurvey[i].image_time}`,
                  question_name: listImageSurvey[i].question_name,
                  item_data: listImageSurvey[i].item_data,
                  survey: listImageSurvey[i].survey_name,
                  image_time: listImageSurvey[i].image_time,
                  _index: i + 1
                })
              }
            }
            ...
            const listImageProduct = (element.image as any == [] as any) ? [] : element.image.product;
            // pr_id: 2
            if (Helper.IsNull(listImageProduct) != true && listImageProduct.length > 0) {
              for (var i = 0; i < listImageProduct.length; i++) {

                photoImageProduct.push({
                  id: i + 1,
                  _id: listImageProduct[i].id,
                  pr_item_id: listImageProduct[i].pr_item_id,
                  pr_id: listImageProduct[i].pr_id,
                  question_id: listImageProduct[i].question_id,
                  src: listImageProduct[i].url,
                  title: `${listImageProduct[i].question_name} - ${listImageProduct[i].image_time}`,
                  question_name: listImageProduct[i].question_name,
                  item_data: listImageProduct[i].item_data,
                  product: `${listImageProduct[i].product_code} - ${listImageProduct[i].product_name}`,
                  image_time: listImageProduct[i].image_time,
                  _index: i + 1
                })
              }
            }

            element.image.survey.listphotoImage = photoImageSurvey
            element.image.product.listphotoImage = photoImageProduct
          ...
          try {
            ...
            const editData = this.dataQC.filter((qc: any) => qc.KPI == 'DISPLAY')[0].is_edit_data;
        ...
        console.log('this.inValue.data_promotion : ', this.inValue.data_promotion)
      })
  }

  loadDataSellOut(report_id: any) {
    this.sellOutService.SellOut_Result_GetList(Helper.ProjectID(), report_id)
      .subscribe((data: any) => {
        if (data.result == EnumStatus.ok) {
          this.inValue.data_sellOut = data.data
          this.inValue.data_sellOut.result.forEach((r: any) => {
            r.details = this.inValue.data_sellOut.details.filter((d: any) => d.sellout_id == r.sellout_id)
            r.image = this.inValue.data_sellOut.image.filter((i: any) => i.sellout_id == r.sellout_id)
            r.master = Helper.IsNull(this.inValue.data_sellOut.master) != true ? this.inValue.data_sellOut.master : []

            r._comfirm_status = (r.comfirm_status == 1) ? true : false
            r.history_confirm = (Helper.IsNull(r.history_confirm) != true) ? JSON.parse(r.history_confirm) : []
            ...
            r.details.forEach((item: any) => {
              ...
              item.history = Helper.IsNull(item.history) != true ? JSON.parse(item.history) : []
            ...
            if (Helper.IsNull(listImage) != true && listImage && listImage.length > 0) {
              for (var i = 0; i < listImage.length; i++) {
                listphotoImage.push({
                  id: i + 1,
                  src: listImage[i].url,
            ...
            if (Helper.IsNull(r.image) != true) {
              r.image.listphotoImage = Helper.IsNull(listphotoImage) != true ? listphotoImage : []
            }

          });

          try {
            const editData = this.dataQC.filter((qc: any) => qc.KPI == 'SELLOUT')[0].is_edit_data;
  ...
  loadDataSellIn(report_id: any) {
    this.sellInService.SellIn_Result_GetList(Helper.ProjectID(), report_id)
      .subscribe((data: any) => {
        if (data.result == EnumStatus.ok) {
          this.inValue.data_sellIn = data.data
          this.inValue.data_sellIn.result.forEach((r: any) => {
            r.details = this.inValue.data_sellIn.details.filter((d: any) => d.sellin_id == r.sellin_id)
            r.image = this.inValue.data_sellIn.image.filter((i: any) => i.sellin_id == r.sellin_id)
            r.master = Helper.IsNull(this.inValue.data_sellIn.master) != true ? this.inValue.data_sellIn.master : []

            r._comfirm_status = (r.comfirm_status == 1) ? true : false
            r.history_confirm = (Helper.IsNull(r.history_confirm) != true) ? JSON.parse(r.history_confirm) : []

            const listImageType = [] as any
            if (Helper.IsNull(this.inValue.data_sellIn.type_image) != true) {
              this.inValue.data_sellIn.type_image.forEach((t: any) => {
                listImageType.push({
                  id: t.Id,
                  code: t.Code,
            ...
            if (Helper.IsNull(listImage) != true && listImage && listImage.length > 0) {
              for (var i = 0; i < listImage.length; i++) {
                listphotoImage.push({
                  id: i + 1,
                  src: listImage[i].url,
                  title: (Helper.IsNull(listImage[i].product_id) != true) ? `[${listImage[i].product_id}] - ${listImage[i].product_code} - ${listImage[i].product_name} | ${listImage[i].app_time} `
                    : `${listImage[i].image_type} - ${listImage[i].NameVN} | ${listImage[i].app_time}`,
                  product: (Helper.IsNull(listImage[i].product_id) != true) ? `[${listImage[i].product_id}] - ${listImage[i].product_code} - ${listImage[i].product_name}` : null,
                  alt: listImage[i].alt,
                  image_time: listImage[i].app_time,
                  _idDetails: listImage[i].id,
          ...
          try {
            const editData = this.dataQC.filter((qc: any) => qc.KPI == 'SELLIN')[0].is_edit_data;

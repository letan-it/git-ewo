...
@Component({
  selector: 'app-works-sellout',
  templateUrl: './works-sellout.component.html',
  styleUrls: ['./works-sellout.component.scss']
})
export class WorksSelloutComponent {

  items_menu: any = [
    { label: ' REPORT' },
    {
      label: ' Works Sell Out',
      icon: 'pi pi-table',
    },
  ];
    ...
    this._serviceSurvey
      ...
      .subscribe((data: any) => {
        ...
        if (data.result == EnumStatus.ok) {
          this.NofiResult('Page Works', 'Import Model Edit', 'Import successful model edit', 'success', 'Successfull');
        } else {
  ...
  ngOnInit() {
    ...
    if (Helper.ProjectID() == 4 || Helper.ProjectID() == 41 || Helper.ProjectID() == 42) {
      this.itemsData.push(
        { separator: true },
        // {
        //     label: 'Raw data pdf',
  ...
  resetLoadTotal(event: any) {
    this.ListReport.forEach((element: any) => {
      if (element.report_id == event.report_id) {
        element.amount = event.sumTotal
      }
    });
  }
